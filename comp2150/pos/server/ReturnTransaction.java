 ...
 *
 * REMARKS: This class extends Transaction class, and creates an instances of the Return Transaction
 *
 */
...
public class ReturnTransaction extends Transaction{
    ...
    //
    public ReturnTransaction(A5POSServer.TransactionType type, long time, int client){
        super(type, time, client);
        idOfTransaction="R"+idNumber;
    ...
    public void addItem(String name, int number, int cost){
        if(itemsList.size()==0){
            ...
            for (int i = 0; i < itemsList.size(); i++) {
                if (itemsList.get(i).getItemName()==name) {
                    index=i;
                }
            }
            if(index>(-1)){
    ...
    //
    public String getNameOfItem(int number){
        String name=null;
        name=itemsList.get(number).getItemName();
        return name;
    }
    ...
    //
    public int getAmountOfItem(int number){
        int amount=0;
        amount=itemsList.get(number).getQuantity();
        return amount;
    }
}
